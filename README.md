...
Бенчмарки тарантула на средненьком (моём) ноутбуке (Air 2012-го года выпуска) на
размере сообщения 220 байт показывают производительность около 180 тыс записей
в секунду, что почти вдвое больше чем нам требуется для данной задачи.
